...
export default function CropModal({ imageUrl, aspectRatio, onConfirm, onCancel }: CropModalProps) {
  ...
  const handleCrop = useCallback(() => {
    ...
    const base = displayInfoRef.current;
    if (base.width === 0) return;

    const scaleFactor = base.naturalWidth / base.width;

    const scaledWidth = base.width * zoom;
    const scaledHeight = base.height * zoom;
    const centerX = base.offsetX + base.width / 2;
    const centerY = base.offsetY + base.height / 2;
    ...
    const clampedX = Math.max(0, Math.min(base.naturalWidth - sourceWidth, sourceX));
    const clampedY = Math.max(0, Math.min(base.naturalHeight - sourceHeight, sourceY));
    const clampedWidth = Math.max(1, Math.min(sourceWidth, base.naturalWidth - clampedX));
    const clampedHeight = Math.max(1, Math.min(sourceHeight, base.naturalHeight - clampedY));
    ...
    onConfirm(base64);
  }, [cropBox, imgOffset, zoom, onConfirm]);
